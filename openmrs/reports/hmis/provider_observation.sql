select concat(coalesce(pn.given_name,''),' ',coalesce(pn.middle_name,''),' ',coalesce(pn.family_name,''))as 'Provider Name',  count(DISTINCT(obs_id)) as Obsevation_Count
from provider pr
  ...
  LEFT JOIN obs o on o.encounter_id = ep.encounter_id and o.voided=FALSE
                     and cast(o.obs_datetime as DATE) between '#startDate#' and '#endDate#'
GROUP BY  pr.provider_id order by Obsevation_Count desc  ;